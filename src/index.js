...
function createMcpServer() {
    ...
    // --- HEVY TOOLS ---
    server.tool("create_routine", "Create a new workout routine", {
    ...
    server.tool("update_routine", "Update an existing workout routine", {
    ...
    server.tool("get_routines", "Fetch the user's saved routines.", {}, async () => {
        try {
        ...
        try {
            ...
            if (totalPages > 1) {
                ...
                for (let i = 2; i <= totalPages; i++) {
                    pageRequests.push(fetch(`https://api.hevyapp.com/v1/exercise_templates?page=${i}`, {
                        headers: { "api-key": HEVY_API_KEY }
                    }).then(r => r.json()));
                }
            ...
            const results = allExercises.filter((ex) => (ex.title || "").toLowerCase().includes(query));
            return {
                content: [{
                        type: "text",
