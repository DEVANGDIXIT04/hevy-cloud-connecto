...
const typeKeywordMapper = {
	...
	object: (nodes) => {
		if (!nodes?.length) return keywordTypeNodes.object;
...
function Type({ name, typedName, tree, keysToOmit, schema, optionalType, arrayType, syntaxType, enumType, enumTypeSuffix, enumKeyCasing, mapper, description }) {
	...
	if (["asConst", "asPascalConst"].includes(enumType) && enumSchemas.length > 0) {
		const isDirectEnum = schema.type === "array" && typeof schema.items === "object" && schema.items !== null && !Array.isArray(schema.items) && "enum" in schema.items;
...
//# sourceMappingURL=components-BlEQhr8A.cjs.map